...
class SepConv(AbstractPrimitive):
    ...
    def get_op_name(self):
        ...
        op_name = op_name + "{}x{}".format(self.kernel_size, self.kernel_size)
        return op_name
...
class Stem(AbstractPrimitive):
    ...
    def __init__(self, C_in=64, C_out=64, **kwargs):
        super().__init__(locals())
        self.seq = nn.Sequential(
            nn.Conv2d(64, C_out, 3, padding=1, bias=False), nn.BatchNorm2d(C_out)
        )
    def forward(self, x, edge_data=None):
        return self.seq(x)
...
class InputProjection(AbstractPrimitive):
    ...
    def get_op_name(self):
        op_name = super().get_op_name
        op_name = op_name + f"{self.module.get_op_name()}"
        return op_name
