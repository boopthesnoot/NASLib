...
class NasBenchSpikingASRSearchSpace(Graph):
    ...
    def get_edge_info(self):
        edge_info = {}
        alpha_idx = 0
        for graph in self._get_child_graphs(single_instances=True):
            for u, v, edge_data in graph.edges.data():
                # if edge_data.has("alpha"):
                if hasattr(edge_data.op, "__len__"):
                    cell_name = edge_data['cell_name'] if hasattr(edge_data, 'cell_name') else ""
                    edge_info[(u, v, cell_name)] = {}
                    if edge_data.op:
                        edge_info[(u, v, cell_name)]['op_names'] = [str(op) for op in edge_data.op]
                    alpha_idx += 1
        return edge_info
    ...
    if use_norm and edge.head == 7:
        ...
        edge.data.set(
            'op', [
                ...
                ops['relu'](filters, filters),
                ops['sigmoid'](filters, filters),
                ops['tanh'](filters, filters),
                ops['absolute'](filters, filters),
